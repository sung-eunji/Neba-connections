...
function reactComponentTagger({
  ...
  return {
    ...
    async transform(code, id) {
      if (id.includes("node_modules") || !extensions.includes(path.extname(id))) {
        return;
      }
            ...
            const attributes = node.attributes.reduce((data, attr) => {
              if (isJSXAttribute(attr) && isJSXIdentifier(attr.name)) {
                if (attr.name.name === "key") {
                  ...
                  } else if (isJSXExpressionContainer(attr.value) && isStringLiteral(attr.value.expression)) {
                    data.push(
...
function isMapCall(node) {
  return isCallExpression(node) && isMemberExpression(node.callee) && isIdentifier(node.callee.property) && node.callee.property.name === "map";
}
function getComponentName(name) {
  return isJSXIdentifier(name) ? name.name : isJSXMemberExpression(name) ? `${getComponentName(name.object)}.${getComponentName(
    name.property
  )}` : "";
}
export {
